...
# --- Load and clean data ---
file_path = "Data.xlsx"
df = pd.read_excel(file_path, sheet_name="Data_FE")
df.dropna(subset=["Latest Fixing", "EUR/GBP", "USD/JPY", "Spot USD/TND"], inplace=True)

# --- PCA: Understanding variance ---
features = ["EUR/GBP", "USD/JPY"]
...
# Prepare features
X_restricted = df[["EUR/GBP", "USD/JPY"]]
...
df["Predicted Spot USD/TND"] = (
    df["Latest Fixing"] +
    (df["EUR/GBP"] * model_restricted.params["EUR/GBP"]) +
